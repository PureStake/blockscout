  ...
  defp reducer({:token_transfers_params, token_transfers_params}, initial) when is_list(token_transfers_params) do
    ...
    |> Enum.reduce(initial, fn
      ...
      when is_integer(block_number) and is_binary(from_address_hash) and
             ...
             token_type == "ERC-20" ->
        acc
        |> add_token_balance_address(from_address_hash, token_contract_address_hash, nil, token_type, block_number)
        |> add_token_balance_address(to_address_hash, token_contract_address_hash, nil, token_type, block_number)
    end)
  end
